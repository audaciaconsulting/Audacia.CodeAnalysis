...
public class TestClass
{
    ...
    public void " + methodName + @"()
    {
        ...
        [DataRow("^[a-zA-Z]+(_[a-zA-Z]+)+$", "MethodName")]
        [DataRow("\\d+", "MethodName")]
        public void Diagnostic_For_Theory_Method_Name_Not_Consistent_With_Configured_Format(string pattern, string methodName)
        {
            SetupConfiguredFormat(pattern);
...
public class TestClass
{
    ...
    public void " + methodName + @"(int value)
    {
            ...
            var expected = BuildExpectedResult(14, 17, methodName, pattern);
            VerifyDiagnostic(testCode, expected);
        }
